	...
	public function request($url, $headers = array(), $data = array(), $options = array()) {
		...
		if (isset($options['verify'])) {
			if ($options['verify'] === false) {
				...
				curl_setopt($this->handle, CURLOPT_SSL_VERIFYPEER, TRUE);   //default value is TRUE
		...
		if (isset($options['verifyname']) && $options['verifyname'] === false) {
			//curl_setopt($this->handle, CURLOPT_SSL_VERIFYHOST, 0);
                        curl_setopt($this->handle, CURLOPT_SSL_VERIFYHOST, 2); // default value is 2 to "check the existence of a common name and also verify that it matches the hostname provided" according to PHP's documentation
		}
