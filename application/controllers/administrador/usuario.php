...
class Usuario extends CI_Controller {
    ...
    public function lista() {
        ...
        $usuarios['from'] = base64_encode(" 
            FROM 
                admin_usuario u
                INNER JOIN admin_ciudad c ON u.id_admin_ciudad = c.id_admin_ciudad
                INNER JOIN admin_departamento d ON c.id_admin_departamento = d.id_admin_departamento
                INNER JOIN admin_pais p ON d.id_admin_pais = p.id_admin_pais
    ...
    public function agregar($id_admin_usuario = NULL, $alert = NULL) {
        ...
        if ($post) {
            ...
            redirect('/administrador/usuario/agregar/' . $id_admin_usuario . '/ok');
        }
        ...
        if ($id_admin_usuario != NULL) {
            $data_body['admin_usuario'] = $this->modelo->join(
                    array(
                '2' => array('admin_departamento', '3'),
                '3' => array('admin_ciudad', '4'),
                '4' => array('admin_usuario', NULL)
                    ), " AND admin_usuario.id_admin_usuario = $id_admin_usuario "
            );
        ...
        $data_body['pais'] = $this->modelo->getAdmin_pais();
