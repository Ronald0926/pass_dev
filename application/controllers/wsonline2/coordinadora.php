...
class Coordinadora extends CI_Controller {
    ...
    public function generarGuia() {
        ...
        $dominioCoordinadora = $this->db->query("select VALOR_PARAMETRO from modgeneri.gentblpargen where PK_PARGEN_CODIGO = 98");
        $dominioCoordinadora = $dominioCoordinadora->result_array[0];
        $dominioCoordinadora = $dominioCoordinadora['VALOR_PARAMETRO'];
        
        log_info($logHeader.$dominioCoordinadora);
        
        $nombre_remitente = $_POST['NOMBRE_REMITENTE'];
        $id_cliente_coor = $_POST['IDCLIENTE'];
        $direccion_remitente = $_POST['DIR_REMITE'];
        $telefono_remitente = $_POST['TELEFONO_REMITENTE']; //revisar
        ...
        $referencia = $_POST['REFERENCIA'];// dejar en blanco
        ...
        log_info($iniciLog . $this->session->userdata("usuario"));

        log_info($logHeader . 'INGRESO  SOLICITUD GUIA COORDINADORA');

        $url = $dominioCoordinadora; //"http://sandbox.coordinadora.com/agw/ws/guias/1.6/server.php";
        log_info($logHeader . $dominioCoordinadora);
   ...
   <soapenv:Body>
      <ser:Guias_generarGuia soapenv:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
        ...
        //log_info($logHeader .$input_xml);

        try {
			log_info($logHeader . ' INICIO COORDINADORA' );
            $pdf_content = $this->get_web_page($url, $input_xml);
           		log_info($logHeader . ' INFO RESPUESTA COORDINADORA' . $pdf_content);
        } catch (Exception $ex) {
            log_info($logHeader . ' ERROR RESPUESTA COORDINADORA' . $ex->getMessage());
        }
...
// extrae el pdf en codificacion base64
        ...
        try {
            ...
            log_info($logHeader . ' ' . $url . '+++' . $codigo_remision);
        } catch (Exception $e) {
            echo 'Excepción capturada: ', $e->getMessage(), "\n";
            log_info($logHeader . ' ERROR ' . $url . '+++' . $codigo_remision);
        }
    }

    public function get_web_page($url, $input_xml) {
        log_info($logHeader . ' PETICION PAGINA WEB COORDINADORA ');
        $options = array(
