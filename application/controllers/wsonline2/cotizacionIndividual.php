...
class cotizacionIndividual extends CI_Controller {
	...
	SELECT pk_entidad_cliente ,comercial.NOMBRE||' '||comercial.apellido COMERCIAL
			FROM MODCOMERC.COMTBLCOTIZAINDIVIDUAL CO
			join MODCOMERC.COMTBLPROCESIND pr
			on co.pk_cotiza_ind_codigo = pr.pk_cotiza_ind_codigo
			join modcliuni.clitblentida comercial
			ON comercial.PK_ENT_CODIGO=co.PK_ENTIDAD_ASESOR
			where pr.pk_proces_ind_codigo = $parpkproceso ");
            $EntidadCliente = $EntidadCliente->result_array[0];
            $dataInfo['comercial'] = $EntidadCliente['COMERCIAL'];
            $EntidadCliente = $EntidadCliente['PK_ENTIDAD_CLIENTE'];
	...
	SELECT 
            ...
            par.PK_PARAMETRO_CODIGO PK_PARAME_CODIGO,
            par.DESC_PARAMETRO PARAMETRO,
            bancos.NOMBRE_BANCO,
            bines.BIN,
            ...
            from  MODCOMERC.COMTBLPARAMEIND par 
            left join  MODCOMERC.comtblvaloresind val on par.PK_PARAMETRO_CODIGO= val.PK_PARAMETRO_CODIGO 
            ...
            //consulta servicios 
               $sqldataServicios = $this->db->query("SELECT 
                ...
                par.PK_TIPPAR_CODIGO PK_TIPPAR_CODIGO,
                par.PK_PRODUCTO_CODIGO
                from  MODCOMERC.COMTBLPARAMEIND par left join  MODCOMERC.comtblvaloresind val 
                on par.PK_PARAMETRO_CODIGO= val.PK_PARAMETRO_CODIGO 
                left join MODPRODUC.PROTBLTIPPRO tp on tp.pk_tippro_codigo=par.pk_tippar_codigo 
                left join MODPRODUC.PROTBLTIPCOS tc  on  val.PK_TIPCOS_CODIGO=tc.pk_tipco_codigo
                where  par.PK_PROCES_CODIGO = $parpkproceso
                and tp.pk_tippro_codigo=3 and par.pk_producto_codigo in(70,999994,999993)
                ORDER BY par.DESC_PARAMETRO desc");
            ...
            $contenido = $this->load->view('/wsonline2/cotizacion/pdfCotizacionIndividual', $dataInfo, TRUE);
            ...
            $sql = " 
                    BEGIN MODCOMERC.compkgfunciones.prcinsertarlinkcotizacionind(
